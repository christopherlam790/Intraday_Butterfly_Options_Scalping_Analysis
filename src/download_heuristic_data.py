"""
PGSQL HEURISTIC DATA -> PYTHON DF
"""
...
"""
Pull heuristic data from table name, if it exists, from PGSQL db with fields specified
...
def get_raw_df_from_sql(table_name: str, fields: list = []) -> pd.DataFrame:
    ...
    try:
        ...
        query = f"""
        SELECT session, {', '.join(fields) if fields else '*'}
        FROM {table_name}
        ORDER BY session;
        """
...
if __name__ == "__main__":
    df = get_raw_df_from_sql("spy_2025_5_minute_ta_heuristics")
